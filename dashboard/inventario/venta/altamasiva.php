...
if (isset($_FILES['subirarchivo']) && $_FILES['subirarchivo']['error'] == 0) {
    ...
    foreach ($worksheet->getRowIterator() as $row) {
        ...
        $stmt = $pdo->prepare('INSERT INTO inventario_venta(nombreproducto,descripcionproducto,codigoproducto,idbodega,idproveedor,precio_compra,precio_venta,stock) VALUES (?,?,?,?,?,?,?,?)');
        $stmt->execute($data);
    }
